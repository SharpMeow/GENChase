...
DU = arb(os.environ.get('NF_DU', '0.02'))           # U-range half width of the block
...
def main(which, T_enter):
    ...
    elif which.startswith('custominterval:'):
        # negative control:  custominterval:<num1>:<num2>:<exp10>  -> c in [num1, num2] / 10^exp10
        _, n1, n2, ex = which.split(':')
        cc = arb(fmpq(int(n1), 10 ** int(ex))).union(arb(fmpq(int(n2), 10 ** int(ex))))
    else:
    ...
    T6m = T6(T)
    log = {'which': which, 'params': nf.PARAMS_TXT, 'prec': ctx.prec, 'c': cc.str(40), 'block': binfo, 'manifold': {k: minfo[k] for k in ('rho', 'r', 'ok')},
    ...
    if which == 'interval' or which.startswith('custominterval:') or not inB:
...
if __name__ == '__main__':
    which = sys.argv[1]
    T_enter = int(sys.argv[2]) if len(sys.argv) > 2 else 45
    main(which, T_enter)
