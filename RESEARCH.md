...
### 2026-09-26  finite rank windows and the eigenspectrum smoothness bound (Stringer et al. 2019): prior articles for the methods note  (session agents; `papers/rank-window/`)

- Why: the neuroscience scout's first candidate was a re-test of the 1/n eigenspectrum and of the smoothness bound alpha >= 1 + 2/d; a stage-1 pilot showed that a finite rank window cannot decide it, and a methods note was written instead.
- Read, in the parts the note uses: Stringer, Pachitariu, Steinmetz, Carandini and Harris, Nature 571 (2019), main text and Supplementary Information (SI Example 3 already says that with equal tuning radius the spectrum is "approximately flat until the eigenfunctions' spatial scale matches this radius"; SI Sect. 2.3 and the Discussion make the strong claims the note addresses); Pospisil and Pillow, PNAS (2025) ("it remains unclear whether the tail of the eigenspectrum decays as a power law"; MEME "can infer eigenvalues beyond the rank of the data"); Davidovich and Roudi, arXiv:2204.08525 (exponents depend on the range of data chosen); Braun, JMLR 7 (2006), Lemmas 5 and 8 (the kernel-matrix bound behind Proposition 1); Shawe-Taylor et al., IEEE Trans. Inf. Theory 51 (2005), Sects. I-III; Kong and Valiant, Ann. Statist. 45 (2017) (arXiv v5, Sects. 1 and 3); Spigler, Geiger and Wyart (arXiv:1905.10843, Sects. 1 and 7). Only the abstract: Koltchinskii and Gine, Bernoulli (2000). Not reached: Widom (1963), HTTP 403.
- Result: the central point (a finite window cannot establish an asymptotic exponent) is elementary and partly anticipated by these sources; the note says so and claims only the quantitative computations on Stringer's own stimulus sets, the d = 1 split into pre-asymptotic and aliasing parts, and the insensitivity of MEME's tail exponent and misfit to the unresolved tail. Two in-project referee readings (`papers/rank-window/notes/review-1.md`, `review-2.md`).
- Re-search: no.

### 2026-09-26  the MICrONS "cohort" wiring rule: reanalysis against degree-preserving nulls and a held-out test on 625 new axons  (session agents; `research/microns-cohort/`)

- Why: the neuroscience scout's second candidate. Read in full: Ding et al., Nature 640 (2025), with its Methods (their Extended Data Fig. 14 already conditions on axon-dendrite co-travel at the synapse scale); the MICrONS Explorer and CAVE documentation for the data access. Code: github.com/cajal/microns-funconn-2025 (MIT).
- Result: the reported DT cohort excess reproduces (+0.0148); under the strictest null (every cell's and axon's totals and each axon's laminar profile fixed) +0.0067 [-0.0008, 0.0134] remains; on 625 held-out axons the pre-registered primary statistic is +0.0014 (z 1.23): **no replication**. The excess is a test statistic, not an effect size (the nulls recover 16 to 47% of an injected rule). Exploratory, one animal; the calibration and held-out work has not been independently checked.
- Data licence: none stated in the sources read; the AWS registry entry for the BossDB bucket lists CC BY 4.0, CC0 and CC BY-NC-SA 4.0 without saying which covers MICrONS. Per-cell derived tables are withheld until it is confirmed.
- Re-search: no.
